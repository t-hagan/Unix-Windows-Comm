...
public:
    XPCContainer **ContainerList;    // The list of CContainer objects

    // Constructor.  Sets the message type, allocates memory for a specified number of CContainer objects,
    ...
    XPCMessage(int _iNumParts, XPCTcpSocket *_Socket, char *_sSubscription) 
    {
        ...
        // The number of XPCContainder objects specified are received
        for (int iCount = 0; iCount < _iNumParts; iCount++)
        {
            ContainerList[iCount] = new XPCContainer();
            int iNumBytes = _Socket->iRecieveMessage((void *)ContainerList[iCount], sizeof(XPCContainer), MSG_WAITALL);
            if (iNumBytes == 0)
            {
