    ...
    register SVCXPRT *transp;
{
    ...
    // If the procedure requested by the client is valid, the RPC server spawns a
    // child process to handle the client's request.  The parent process returns
    // enabling the RPC server to process additional client requests
    pid = fork();
    if (pid > 0) 
        return;
    ...
    if (!svc_getargs(transp, _xdr_argument, (caddr_t) &argument)) {
        ...
        // The RPC server child process exits
        exit(0);    
    }
