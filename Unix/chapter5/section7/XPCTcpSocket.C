...
&iClientAddressLen)) == -1)
    {
        ...
        sprintf(sMsg, "Error Accepting Socket. %s\0", strerror(errno));
        XPCException socketExcept(sMsg);
        throw socketExcept;
        return NULL;
    } 
...
int XPCTcpSocket::iSendMessage(void *_vMessage, int _iMessageSize)
{
    ...
    if ((iNumBytes = send(iSocket, (char *)_vMessage, _iMessageSize, 0)) == -1)
    {
        char sMsg[512];
...
receive
    int iOffsetSize = 0;            // The number of bytes currently recieved
...
continue to
    ...
    {
        // The socket message is recieved and stored within the mesage offset by
the 
...
not thrown.
        ...
        // If the total number of bytes requested are not returned, the offset
is adjusted
...
_iOption)
{
    ...
    if (_iOption == MSG_WAITALL)
        // If the option is MSG_WAITALL and this is a WINDOW_NT machine call
iReceiveMessageAll
        // to process it
        return iRecieveMessageAll(_vMessage, _iMessageSize);
#endif

    // Recieves a TCP socket message.  The number of bytes received is returned
    iNumBytes = recv(iSocket, (char *)_vMessage, _iMessageSize, _iOption);
    if (iNumBytes == -1)
    {
        // If the reason for failure is a client disconnect, an exception is not
thrown.
        // The number of bytes returned is 0    
#ifdef UNIX
        if (errno == ECONNRESET)
...
void XPCTcpSocket::vBindSocket()
{
    ...
    if (bind(iSocket, (struct sockaddr *)&clientAddress, sizeof(clientAddress))
== -1)
    {
        char sMsg[512];
