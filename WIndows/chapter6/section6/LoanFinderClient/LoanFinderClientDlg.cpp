...
void CLoanFinderClientDlg::OnCalcButton() 
{
    ...
    m_BankEdit.SetWindowText(sBuffer);

    sprintf(sBuffer, "%.02f", retpayment->dPayment);
    m_PaymentEdit.SetWindowText(sBuffer);
    ...
    if (retschedual == NULL)
    {
        MessageBox("Error Retrieving the Loan Amortization Schedule", "Error", MB_OK);
        return;
    }
    ...
    while(currentpayment != NULL)
    {
        ...
        m_AmortizationListBox.InsertString(-1, sPaymentString);
